...
## Current release candidate — 0.3.1-rc.1

RC1 changes the **Home Assistant frontend transport** after a real restrictive enterprise-Wi-Fi test. Under beta 8 the WelcomeEye-to-Home-Assistant media path remained healthy, H.264 decoded correctly and the SDP answer was produced, but WebRTC stayed in ICE `checking` with STUN available and no TURN relay. Temporarily disabling the native WebRTC advertisement made the same camera work through Home Assistant's existing Stream/HLS path on that same network.

RC1 therefore stops advertising the integration's native WebRTC handler and lets Home Assistant consume the existing `stream_source()` through its Stream/HLS transport. It requires **no TURN server, no extra container, no external relay and no firewall change**. Initial playback can need a few seconds of buffering before it stabilizes; RC1 intentionally favors broad network compatibility over the lowest possible latency.
...
- No TURN service or additional container required for RC1 remote viewing through Home Assistant.
...
| **WelcomeEye Connect V1 / DES9900VDP** | **Video validated on real hardware** | **RC1 software path validated; physical relay confirmation pending** | **Standby; local path not yet demonstrated** | Experimental / active testing |
...
- after a decoder reset, RC1 can reuse a bounded cached SPS/PPS pair if the next genuine IDR omits those parameter sets;
...
- RC1 uses Home Assistant Stream/HLS rather than the integration's native WebRTC path; startup can take a few seconds while the stream buffers and latency can be higher than WebRTC.
- Native WelcomeEye WebRTC code is retained but deliberately not advertised by the camera in RC1.
...
RC1 diagnostics explicitly identify the frontend transport policy without exporting the internal loopback stream URL. The MPEG-TS proxy remains bound only to `127.0.0.1` and uses a randomly generated path for each Home Assistant integration instance.

The native WebRTC implementation remains present but is not advertised by the camera in RC1, so ordinary RC1 camera playback uses Home Assistant Stream/HLS instead of opening a WelcomeEye WebRTC peer connection.
...
Beta 8 was validated with **135 tests plus 3 subtests** on Python **3.12.14** and **3.14.7**, with compilation, HACS and Hassfest all green. RC1 adds focused regression checks for the transport policy, retained MPEG-TS/HTTP lifecycle and manifest/diagnostics version alignment without changing device protocol handling.

The RC1 transport choice was field-tested on the restrictive enterprise Wi-Fi that left beta 8 WebRTC stuck in ICE `checking`: the Home Assistant Stream/HLS path successfully produced live video there, with a short unstable/buffering phase at startup before stabilizing.
