...
- Add a tester-specific regression in which one TLV 505 is sent, the media decoder fails while confirmation is pending, the same worker/session recovers on a fresh keyframe, and a later TLV 506 is accepted with no second TLV 505.
- Add a second regression through the actual V1 hub media worker with real PyAV decoding and the same command/decode-error/recovery/confirmation sequence.
...
- Validate the final candidate with **97 tests plus 3 subtests** on both Python **3.12** and **3.14**, using PyAV 17.0.1 and dnspython 2.8.0. HACS and Hassfest are green on the stability branch.
