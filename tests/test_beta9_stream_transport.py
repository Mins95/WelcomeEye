"""Regression checks for the RC2 Home Assistant Stream transport policy."""
import ast
import json
...
def test_rc2_disables_native_webrtc_advertisement_but_keeps_stream_source():
...
def test_rc2_keeps_existing_http_mpegts_stream_lifecycle():
...
def test_rc2_version_and_diagnostics_policy_are_aligned():
    manifest = json.loads(MANIFEST.read_text(encoding="utf-8"))
    diagnostics = DIAGNOSTICS.read_text(encoding="utf-8")
    assert manifest["version"] == "0.3.1-rc.2"
    assert 'VERSION = "0.3.1-rc.2"' in diagnostics
